...
// Lấy action gốc từ URL để phục vụ cho thuộc tính Action của Form trước khi bị thay đổi phục vụ hiển thị
$formAction = $_GET['action'] ?? 'list';
...
if (($formAction === 'edit' || $action === 'edit') && isset($_GET['id'])) {
...
<body>
                    ...
                    <div class="col-md-4">
                        <div class="card mb-4">
                            <div class="card-body">
                                ...
                                <form method="POST"
                                    action="categories.php?action=<?= $action ?><?= $action === 'edit' ? '&id=' . htmlspecialchars($_GET['id']) : '' ?>">
                                    ...
                                    <div class="d-flex justify-content-between align-items-center" style="gap: 10px;">
                                        <button type="submit"
                                            class="btn btn-<?= $action === 'edit' ? 'warning' : 'success' ?> flex-fill">
                                            <i class="fa fa-save"></i>
                                            <?= $action === 'edit' ? ' Cập nhật' : ' Thêm danh mục' ?>
                                        </button>
                                        <?php if ($action === 'edit'): ?>
                                            <a href="categories.php" class="btn btn-secondary flex-fill">
    ...
    <script>
        // Tìm kiếm realtime có check an toàn tránh crash JS khi bảng trống
        document.getElementById('searchInput').addEventListener('keyup', function () {
            const keyword = this.value.toLowerCase();
            document.querySelectorAll('#categoriesTable tbody tr').forEach(row => {
                if (row.classList.contains('no-data-row')) return; // Bỏ qua dòng thông báo rỗng
